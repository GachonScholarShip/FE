...
function MapViewPage() {
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      const mapOptions = {
        center: new window.naver.maps.LatLng(37.5665, 126.9780), // 예시로 서울 중심
        zoom: 14,
      };
      ...
      const startLocation = new window.naver.maps.LatLng(37.5665, 126.9780);  // 키오스크 위치 예시
      let endLocation = new window.naver.maps.LatLng(37.5700, 126.9850);  // 선택된 건물의 기본 위치

      // 선택된 건물에 맞는 좌표로 수정 필요
      if (selectedBuilding === "가천관") {
        endLocation = new window.naver.maps.LatLng(37.5665, 126.9780); // 가천관의 실제 좌표
      }
      ...
      directionsService.route(
        ...
        (response, status) => {
          if (status === "OK") {  // DirectionsStatus.OK -> 'OK'로 수정
