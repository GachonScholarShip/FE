...
function BuildingManagementUpdatePage() {
  ...
  const { id, name, available, topFloor, bottomFloor } = location.state;
  ...
  const parseFloor = (floorStr) => {
    ...
    if (floorStr.includes("B")) return -parseInt(floorStr.replace("B", ""));
  ...
  const [building, setBuilding] = useState({
    ...
    name: name,
    available: available,
    topFloor: parseFloor(topFloor),
    bottomFloor: parseFloor(bottomFloor),
  });
  ...
  const handleSaveClick = async () => {
    ...
    if (!id) {
      alert("잘못된 요청입니다. ID가 필요합니다.");
      return;
    }

    const data = {
      buildingName: selectedName,
      topFloor: parseInt(selectedTopFloor, 10),
      bottomFloor: parseInt(selectedBottomFloor, 10),
      isPublic: selectedAvailability === "가능",
    };

    try {
      const response = await axios.patch(
        `http://110.15.135.250:8000/building-service/admin/building/${id}`,
        data,
        {
          headers: {
            Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJ1c2VySWQiOjIsInJvbGUiOiJBRE1JTiIsInN1YiI6IkF1dGhvcml6YXRpb24iLCJpYXQiOjE3NDU0MTEzMjUsImV4cCI6MTc1NDA1MTMyNX0.V0RNqeCd4j7XznWD6c5x1wVhf4QNwoXGKhgWa2C5rJs`, // 유효한 토큰인지 확인
    ...
    <div>
      ...
      <div className={styles.bg}>
        <div className={styles.container}>
          ...
          <div className={styles.bottomBox}>
            <div className={styles.title}>건물 수정</div>
            <div className={styles.formContainer}>
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  <label>건물명</label>
