...
function BuildingManagementUpdatePage() {
  ...
  const handleSaveClick = async () => {
    ...
    const parsedTop = parseFloor(selectedTopFloor);
    const parsedBottom = parseFloor(selectedBottomFloor);

    if (isNaN(parsedTop) || isNaN(parsedBottom)) {
      alert("층 정보가 잘못되었습니다. 숫자 또는 'B1' 형식으로 입력해주세요.");
      return;
    }
